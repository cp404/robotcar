...
int main()
{
 ///////////////////////////////////各模块初始化///////////////////////////////////
  ...
  while(1)
	{
	 if(powerflag==1)//启动键按下才开始工作
	 {    
//		printf("%d",Distance1);
//		printf ("            ");	
///////////////////舵机控制//////////////////////////
		 
	  TIM_SetCompare1(TIM3,x);//(初始值1870)      //中断中不停检测ps2按键值来改变x,y,z,r的值
	  TIM_SetCompare2(TIM3,y);//(初始值1860)
	  TIM_SetCompare3(TIM3,z);//(初始值1820)
	  TIM_SetCompare4(TIM3,r);//(初始值1840)
		
//		printf("%d",x);//调试用串口通讯程序
//	  printf ("            ");
		 
		 if(traflag==1)//循迹模式下轮训循迹
		 {
			 Auto_Tracking();
		 }
